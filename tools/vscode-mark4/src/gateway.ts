...
// both ways (gateway.proto, see software/hub/README.md). Nothing raw
// crosses: the gateway publishes typed messages and the extension sends
...
export interface GatewayHandlers {
    ...
    /**
     * Log lines of one node: one as it arrives, or the ring the gateway
     * kept when this client connects.
     */
    onLogLines(node: number, lines: readonly Log[]): void;
    ...
    setLogLevel(nodeId: number, moduleId: number, level: LogLevel): void {
        this.send(
            create(GatewayMessageSchema, {
                body: {
                    case: "logCommand",
                    value: { node: nodeId, action: { case: "setLevel", value: { moduleId, level } } },
                },
            }),
        );
    }

    /** Asks the gateway to pull a node's module table again. */
    queryLogModules(nodeId: number): void {
        this.send(
            create(GatewayMessageSchema, {
                body: {
                    case: "logCommand",
                    value: { node: nodeId, action: { case: "refresh", value: true } },
                },
            }),
        );
    }
    ...
    private dispatch(bytes: Uint8Array): void {
        ...
        switch (message.body.case) {
            ...
            case "nodeLogModules":
                ...
                return;
            case "nodeLogLines":
                this.handlers.onLogLines(message.body.value.node, message.body.value.lines);
                return;
            default:
