...
`self` to `src` and counts. The composition builds the `Announce` once,
...
`DiscoveryDirectory` is a `Discovery` (it answers too) that is also an
`AbsPresenceListener` of the transport. A node that appears gets an entry
`PENDING`; the next `tick()` asks it; every `IDENTITY_TIMEOUT_US` (500 ms)
without an answer it is asked again, up to `IDENTITY_RETRIES` (5)
requests, then the entry is `MUTE` and left alone. An `Announce` from the
node, at any time, makes the entry `KNOWN`: the announce is stored, the
`wireMismatch` flag compares its `wire_hash` with this build's `WIRE_HASH`,
`hops` is read from the transport's node. A node the transport expires
...
struct DirectoryEntry
{
    ...
    std::uint64_t askedUs;            // instant of the last request
    std::uint8_t requests;            // requests sent so far
    std::uint64_t updatedUs;          // instant of the last state change
};

DiscoveryDirectory directory(messenger, transport, self);   // after the messenger; it is a presence listener too
...
at once. A request is counted whether or not the frame left: on the very
first tick the transport may not hold the node's address yet, and the
retry covers it. `MAX_ENTRIES` is the transport's `MAX_NODES`; a node that
...
(4), attached in the constructor and detached in the destructor, `init()`
...
| `requests()` | `IdentityRequest`s sent, the refused ones included |
| `learnt()` | `Announce`s stored as an identity |
| `muted()` | nodes given up on after `IDENTITY_RETRIES` requests |
