...
##
## The world keys (hold, throw, reset) drive the followed drone: the first
## one created, then the oldest survivor. The camera, the arena fade and
## the overlay follow it too.
...
## Keyboard requests, handed to the followed drone.
@export var pilot_path: NodePath
@export var camera_path: NodePath
@export var arena_path: NodePath

var transport := Mark4Transport.new()
## Flight process node id -> Drone.
var drones: Dictionary = {}
## Drone the keys, the camera and the overlay follow; null when none.
...
func _ready() -> void:
	...
	_camera = get_node_or_null(camera_path) as FollowCamera
...
func _physics_process(_delta: float) -> void:
	...
	if _pilot == null or followed == null:
		return
	if _pilot.take_reset_request():
		followed.request_reset()
	if _pilot.take_grab_request():
		followed.request_grab()
	if _pilot.take_throw_request():
		followed.request_throw()
